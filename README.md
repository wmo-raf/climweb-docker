...
| CMS_DB_VOLUME            | Mounted docker volume path for persisting database data                                                                                                                                                                                              | YES      |   ./cms/db-data      |                                                                                                       |
...
| SECRET_KEY               | A secret key for a particular Django installation. This is used to provide cryptographic signing, and should be set to a unique, unpredictable value. Django will refuse to start if SECRET_KEY is not set. Sample tool for generating secret keys is https://djecrety.ir/                            | YES      |         |                                                                                                       |
