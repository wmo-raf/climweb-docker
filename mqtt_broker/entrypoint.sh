...
if [ ! -e "/mosquitto/config/password.txt" ]; then
  ...
  mosquitto_passwd -b -c /mosquitto/config/password.txt $CMS_BROKER_USERNAME $CMS_BROKER_PASSWORD
...
sed -i "s#_CMS_BROKER_QUEUE_MAX#$CMS_BROKER_QUEUE_MAX#" /mosquitto/config/mosquitto.conf
sed -i "s#_CMS_BROKER_USERNAME#$CMS_BROKER_USERNAME#" /mosquitto/config/acl.conf
